    ...
    self.addListener('raw', function(message) {
        ...
        switch (message.command) {
            ...
            case 'err_nicknameinuse':
                ...
                if (self.opt.nick.length + self.opt.nickMod.toString().length > 15)
                {
                        var lengthToTrim = self.opt.nick.length + self.opt.nickMod.toString().length - 15;
                        self.send('NICK', self.opt.nick.substring(0, self.opt.nick.length - lengthToTrim) + self.opt.nickMod);
                        self.nick = self.opt.nick.substring(0, self.opt.nick.length - lengthToTrim) + self.opt.nickMod;
                }
                else
                {
                ...
                self.opt.nickMod = self.opt.nickMod + 1;
